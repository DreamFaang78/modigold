...
import {
  ...
  TrendingUp, Users, DollarSign, Menu, X, BarChart2,
} from 'lucide-react';
        ...
        <div className="bg-white p-10 w-full max-w-md shadow-2xl">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="email" className="input-field" value={creds.email}
                onChange={(e) => setCreds({ ...creds, email: e.target.value })}
                placeholder="admin@modigold.in" />
            </div>
            <div>
            ...
            <button onClick={() => setIsLoggedIn(true)} className="btn-gold w-full justify-center py-4">
              Sign In
            </button>
            <p className="text-center text-xs text-gray-400">Demo: any email/password works</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex" style={{ background: '#f8f9fa' }}>
      ...
      >
        ...
        <div className="p-4" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <button onClick={() => setIsLoggedIn(false)} className="w-full flex items-center gap-2 text-white/40 hover:text-white text-xs transition-colors">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        ...
        <header className="bg-white px-6 py-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-bold text-navy-500 capitalize">{activeSection}</h1>
          </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ borderBottom: '1px solid #f1f1f1', background: '#f9fafb' }}>
              {['Product', 'Category', 'SKU', 'Mode', 'Price', 'Stock', 'Actions'].map((h) => (
              ...
              <tr key={p.id} className="hover:bg-gray-50 transition-colors" style={{ borderBottom: '1px solid #f9fafb' }}>
                ...
                <td className="py-3 px-4">
                  <div className="flex gap-2">
                    <button className="p-1.5 hover:text-gold-500 transition-colors text-gray-400"><Eye size={14} /></button>
                    <button className="p-1.5 hover:text-blue-500 transition-colors text-gray-400"><Edit size={14} /></button>
                    <button className="p-1.5 hover:text-red-500 transition-colors text-gray-400"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
...
function SettingsSection() {
  return (
    <div className="max-w-2xl space-y-8">
      {[
        { label: 'Store Settings', fields: [
          { name: 'Store Name', value: 'Modigold Pipes Pvt Ltd' },
          { name: 'Contact Email', value: 'sales@modigold.in' },
          { name: 'WhatsApp Number', value: '+91 9960 937 588' },
        ]},
